...
impl<H: HttpClient> PollClient<H> {
    ...
    pub async fn poll_once(&self) -> Result<PollOutcome, PollError> {
        let request =
            HttpRequest::get(self.config.path.clone()).with_timeout(self.config.poll_timeout);
        let scheduler_guard_timeout = self.config.poll_timeout + Duration::from_secs(1);
        let response =
            match tokio::time::timeout(scheduler_guard_timeout, self.http.send(request)).await {
                Ok(result) => result?,
...
mod tests {
    ...
    async fn poll_once_maps_204_to_empty() {
        let http = Arc::new(FakeHttpClient::new(vec![Ok(response(204, None))]));
        let client = PollClient::new(Arc::clone(&http), config());
