...
final class InitiationSyntheticSnapshotTests: XCTestCase {
  func testRenderSyntheticInitiationSurfacesWhenRequested() async throws {
    ...
    //    body's now — `WorkBlockView` stopped drawing it when it turned out to
    //    be reachable only from inside the focus-session popover — so the shot
    //    composes the two surfaces the way the panel does.
    ...
    try render(
      VStack(alignment: .leading, spacing: 0) {
        WorkBlockProactiveCards(coordinator: coordinator)
