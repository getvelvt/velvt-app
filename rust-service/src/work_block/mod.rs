...
use velvt_shared_types::{
    ...
    InterventionResponse, StartWorkBlock, WorkBlockCoverage, WorkBlockDndOutcome,
...
impl WorkBlockManager {
    ...
    fn finish(
        ...
        let result = aggregate_result(record, phase, elapsed, &observations, &corrections);
        // DND evidence is decided once, at finalization, and persists with
        // the result: a block that completes while DND is active is a
        // success (D8 recovery framing), and every held decision reconciles
        // here as a count — the one calm line below is the only surface a
        // suppressed nudge ever gets.
        let held_count = self
            .repo
            .interventions(&record.block_id)?
            .iter()
            .filter(|offer| offer.outcome == WorkBlockInterventionOutcome::DeliverySuppressedDnd)
            .count() as u32;
...
mod tests {
    ...
    fn invited_block_flows_the_unchanged_drift_machinery() {
        ...
        let recorded = repo
            .interventions(&repo.latest().unwrap().unwrap().block_id)
            .unwrap();
    ...
    fn held_nudges_reconcile_after_the_block_as_a_count_only() {
        ...
        let rows = repo.interventions(&block_id.to_string()).unwrap();
    ...
    fn dnd_toggled_mid_block_holds_cooldown_without_touching_backoff() {
        ...
        // Past the base cooldown, fresh evidence earns a normal offer at
        // standard salience: suppression multiplied nothing and lowered
        // nothing.
        ...
        let offer = observe(&manager, "COMMUNICATION", 1_490)
            .unwrap()
            .intervention
            .expect("fresh evidence past the base cooldown offers normally");
        assert_eq!(offer.salience, DriftSalience::Standard);
        ...
        assert_eq!(rows[1].outcome, WorkBlockInterventionOutcome::Offered);
    }
